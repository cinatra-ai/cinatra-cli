...
// --- verifyRegisteredHostnameMatchesPrediction ----------------------------
// SKIPPED in the standalone repo: this block drives the REAL connector helper
// `deriveDevTailscaleHostname`, resolved on demand from the gitignored
// `extensions/cinatra-ai/tailscale-connector` clone (populated only by
// `cinatra setup dev`). That extension source is not present in the extracted
// CLI repo, so these cases require the upstream monorepo's extensions tree and
// belong to its integration suite (codex-converged boundary). The pure helpers
// above (`extractTailscaleHostnameSegment`) and below (`shouldWritePublicBaseUrl`)
// remain fully covered here.

describe.skip("verifyRegisteredHostnameMatchesPrediction (needs extensions clone)", () => {
