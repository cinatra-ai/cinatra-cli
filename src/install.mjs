...
// cinatra-cli#197: the instance's EFFECTIVE local-infra endpoints — the half of
// a dev install's configuration that `.env.example` leaves implicit. Lives in
// its own node-builtins-only module so the preview composition can read the
// SAME derivation without importing this file (which lazy-imports it).
import {
  composePublishedPortsForTarget,
  deriveEffectiveInstanceEndpoints,
  infraPlanOwnsLocalStack,
  parseComposePublishedPorts,
} from "./instance-endpoints.mjs";
// cinatra-cli#198: which working-tree dirt a completed install produced itself,
// so the clean-tree guard keeps refusing OPERATOR changes without refusing the
// installer's own deterministically-regenerated byproducts.
import {
...
/** As `capture`, but WITHOUT trimming — for output whose leading/trailing bytes
 *  are significant. `git status --porcelain` is exactly that: the first two
 *  columns are the status code, so " M path" trimmed becomes "M path" and every
 *  subsequent offset is wrong by one (cinatra-cli#198). */
function captureRaw(command, args, { cwd, env } = {}) {
  try {
...
export function parseInstallArgs(argv = []) {
  ...
  // checkout already parked at an exact commit and required to hand it back
  // byte-for-byte clean. Each one is OFF by default and
  // changes NOTHING for anyone who does not ask for it; the hand-run install is
  // unchanged to the character.
  //
  // `--pinned-extensions` pins the DEV extension fleet to the checkout's own
  // committed lock shas (fail-closed: an entry that cannot be pinned stops the
  // install rather than floating to a tip). A prod install never syncs that
  // fleet — it acquires its required extensions pinned and integrity-verified
  // on its own path — so accepting the flag there would let an operator believe
  // they had pinned something. Refused HERE, before any side effect, in the
  // same position as the mode-scoped refusals above.
  const pinnedExtensions = argv.includes("--pinned-extensions");
  if (pinnedExtensions && !isDevLikeMode(mode)) {
    throw new Error(
      `--pinned-extensions applies only to a dev-like install (dev|demo|preview) — it pins the DEV ` +
        `extension fleet to the checkout's committed lock, and a ${surfaceMode} install acquires its ` +
        `extensions pinned + integrity-verified on its own path.`,
    );
  }
  ...
  return {
    ...
    // The unattended opt-ins (validated above). Dev-like only for the first;
    // the other two apply to every mode, because every mode installs
...
// `parseComposePublishedPorts` / `composePublishedPortsForTarget` now live in
// `instance-endpoints.mjs` (cinatra-cli#197) so the preview composition can
// derive the SAME band without importing this module. Re-exported here because
// this file is their established public surface.
export { parseComposePublishedPorts };
...
/** cinatra-cli#198 — classify `dir`'s working-tree dirt into the install's own
 *  byproducts vs operator changes. Replaces the pre-#198 boolean
 *  `workingTreeIsDirty`, which could only COUNT dirt and so refused the
 *  installer's own output. A `git status` that cannot be captured is reported
 *  CLEAN, exactly as that predicate already treated it (a git failure is
 *  surfaced by the fetch/move that follows, not invented here). */
function classifyTargetDirt(dir, deps = {}) {
  const cap = deps.captureRaw ?? captureRaw;
  const out = cap("git", ["-C", dir, "status", "--porcelain", "-z"], { env: gitEnv() });
  return classifyWorkingTreeDirt(out ?? "");
}
...
export function ensureEnvLocal({ targetDir, mode, resetEnv = false, log = console.log }) {
  ...
  if (existsSync(envPath) && !resetEnv) {
    ...
    if (profileChanged || secretsChanged) {
      writeFileSync(envPath, withSecrets, { mode: 0o600 });
      tightenEnvLocalPerms(envPath);
      const profileNote = profileChanged
        ? profile
...
function ensureInstanceSecrets({ body, envPath, sourceValues = {} }) {
  ...
  if (attestExisting) {
    next = upsertEnvKey(next, ATTEST, attestExisting);
    preserved.push(ATTEST);
  } else {
...
/** What `--frozen-lockfile` adds to a dependency install, resolved in ONE place
 *  so no package-manager tier can silently drop the operator's opt-in. */
function frozenLockfileParts(frozenLockfile) {
  return frozenLockfile
    ? { args: ["--frozen-lockfile"], suffix: " --frozen-lockfile" }
    : { args: [], suffix: "" };
}
 ...
 *
 *  `frozenLockfile` appends `--frozen-lockfile` on EVERY tier, so an unattended
 *  install refuses a lockfile drift instead of rewriting a tracked file under a
 *  caller that has to hand its checkout back clean. Default false — the
 *  invocation is then byte-for-byte what it has always been. */
export function resolvePnpmInvocation({ targetDir, exists = commandExists, frozenLockfile = false } = {}) {
  const frozen = frozenLockfileParts(frozenLockfile);
  if (exists("corepack", ["--version"])) {
    return {
      command: "corepack",
      args: ["pnpm", "install", ...frozen.args],
      label: `corepack pnpm install${frozen.suffix}`,
    };
  }
  if (exists("pnpm", ["--version"])) {
    return { command: "pnpm", args: ["install", ...frozen.args], label: `pnpm install${frozen.suffix}` };
  }
  const spec = pinnedPnpmSpec(targetDir);
  if (spec && exists("npm", ["--version"])) {
    return {
      command: "npm",
      args: ["exec", "-y", "--", spec, "install", ...frozen.args],
      label: `npm exec -y -- ${spec} install${frozen.suffix}`,
      pinned: spec,
    };
  }
  return {
...
/** The argv of the `instance setup` child, built in ONE place so no forwarded
 *  opt-in can be dropped by one caller and kept by another.
 *
 *  The command-routing contract (renamed cinatra-cli#61): the CANONICAL
 *  namespaced form (`cinatra instance setup <mode>`) — the only form that
 *  resolves (the bare `setup <mode>` was removed in cinatra-cli#81).
 *
 ...
 *
 *  `pinnedExtensions` forwards the install's opt-in as the child's own
...
export function runSetupInTarget({
  ...
  if (!verdict.tolerated) {
    // cinatra-cli#270 — one of the failing statuses is TYPED: the setup phase
    // refused to rewrite the generated extension maps because they do not
    // match this checkout's pinned extension set. It stays a FAILURE (the run
    // stopped and nothing was written), so it is never tolerated — but the
    // caller this opt-in exists for is unattended and reads the exit code, and
    // a bare 1 makes "your committed maps are stale, regenerate and commit
    // them" indistinguishable from a Docker that would not start. So the
    // child's own code is re-raised as this process's, the same shape the
    // install tail uses for 20 and 21; `bin/cinatra.mjs` honours
    // `error.exitCode`. The drifted files are already in front of the operator
    // — the child ran on inherited stdio and named every one of them.
    //
    // Fail CLOSED on the mode, exactly as `canReportRegistrySkew` does: only
    // the DEV setup child is given `--pinned` (setupChildArgs), so only it can
    // mint this status. The same code out of a prod child is an unexplained
    // failure, not a drift, and must not borrow the meaning.
    if (setupMode === "dev" && result.status === SETUP_EXIT_GENERATED_MAPS_DRIFT) {
      const drift = new Error(
        `${message} The generated extension maps under src/lib/generated/ do not match this checkout's ` +
          `pinned extension set, and setup refused to rewrite them — see the files it named above. ` +
          `Regenerate them and commit them on the commit this checkout is parked at, then re-run. ` +
          `Every tracked file is exactly as it was handed over.`,
      );
      drift.exitCode = SETUP_EXIT_GENERATED_MAPS_DRIFT;
      throw drift;
    }
...
async function executeCoUse({ targetDir, opts, resolvedSha, log = console.log, deps = {} }) {
  ...
  if (persisted.idempotent) {
    ...
    // rollback (nothing was provisioned this run). Prod-only.
  ...
  try {
    if (isDevLikeMode(opts.mode) && deps.skipCoUseInstall !== true) {
      ...
      const extResult = await (deps.syncDevExtensions ?? syncCinatraDevExtensions)({
        repoRoot: targetDir,
        targetRoot: targetDir,
        // Same opt-in as the default path — co-use owns its whole tail, so it
        // has to carry the flag itself or a co-use instance would silently get
        // a tip-tracking fleet where the default path pinned one.
        argv: opts.pinnedExtensions ? ["--pinned"] : [],
        env: process.env,
        log,
      });
    ...
    if (!opts.noInstall && deps.skipCoUseInstall !== true) {
      ...
      (deps.pnpmInstall ?? pnpmInstall)({
        targetDir,
        usePnpmDirect: usePnpm,
    ...
    // donor — buildCoUseEnv/writeCoUseEnv (above) copy them into .env.local
    // UNCONDITIONALLY — and it never mints its own. Unlike the default install
    // path, co-use has NO ensureEnvLocal guard to mint-if-missing / abort-on-
    // malformed the hard key, so this gate is the ONLY thing standing between a
    // donor with a missing/invalid CINATRA_ENCRYPTION_KEY and a co-use instance
    // that reports success but crashes on first prod boot. It therefore runs
    // REGARDLESS of --no-setup / --no-install: those flags short-circuit only the
    // deps/setup phases, never the env write, and the instance is marked ready +
    // reported "✓ ready" below either way. VALIDATE the required-env matrix BEFORE
    // marking ready, so a hard failure aborts here and the catch below rolls the
    // created DB + slot back. Mirrors the idempotent-converge gate above. Prod-
    // only; the dry-run path returned above, so this is never reached for a dry run.
    if (RUNTIME_MODE[opts.mode] === "production") {
      assertProdEnvComplete({ targetDir, log });
    }
...
export function parseEnvBody(body) {
  ...
  for (const raw of String(body ?? "").split(/\r?\n/)) {
    ...
    if (!m) continue;
    // dotenv parity (cinatra-cli#143): a QUOTED value is the content between the
    // opening quote and its matching close — any trailing inline comment after the
    // close is ignored, and a `#` INSIDE the quotes is literal. An UNQUOTED value
    // has a trailing inline comment (a `#` preceded by whitespace) stripped; a bare
    // `#` with no leading space stays part of the value. This is what the app's own
    // dotenv loader sees, so `KEY=<hex> # note` and `KEY="<v> # keep"` parse right.
    let v = m[2].trim();
    if (v.startsWith('"') || v.startsWith("'")) {
      const quote = v[0];
      const close = v.indexOf(quote, 1);
      if (close !== -1) v = v.slice(1, close);
      // An unterminated quote is left as-is (matching the prior leniency).
    } else {
...
/**
 * cinatra-cli#143 — the POST-INSTALL validation GATE for `--mode prod`. After the
 * setup subprocess has written/reconciled `.env.local`, re-parse it and validate
 * it against the LOCAL mirror of the app's HARD/SOFT required-env matrix
 * (`prod-env-validate.mjs`). A missing or malformed HARD var THROWS — naming every
 * failing var and the reason, in the app's own `[required-env-preflight]` style —
 * so `install --mode prod` exits non-zero instead of declaring success on an
 * instance that would abort on first prod boot. A missing SOFT var (or the
 * distinct WayFlow `CINATRA_CONTEXT_ATTEST_KEY`) only WARNS; the install still
 * succeeds. Pure w.r.t. the env: it reads the file and never mutates it.
 */
export function assertProdEnvComplete({ targetDir, log = console.log } = {}) {
  const envPath = path.join(targetDir, ".env.local");
  if (!existsSync(envPath)) {
    throw new Error(
      `[required-env-preflight] .env.local is missing at ${envPath} — cannot validate the prod ` +
        `required-env set. The install did not produce an env file; re-run \`cinatra install --mode prod\`.`,
    );
  }
  const env = parseEnvBody(readFileSync(envPath, "utf8"));
  const { hardFailures, softMissing, attestMissing } = checkProdEnv(env);

  if (hardFailures.length > 0) {
    throw new Error(formatHardFailureMessage(hardFailures));
  }

  for (const s of softMissing) {
    log(
      `  ⚠ [required-env-preflight] ${s.name} is not set — ${s.why}. The install is complete, but the ` +
        `dependent feature is unavailable until it is provisioned.`,
    );
  }
  if (attestMissing) {
    log(
      `  ⚠ [required-env-preflight] ${ATTEST_KEY.name} is not set — ${ATTEST_KEY.why}. The install is ` +
        `complete; WayFlow attestation runs degraded until it is provisioned.`,
    );
  }
  log("  ✓ [required-env-preflight] prod required-env matrix validated in .env.local.");
...
function effectiveIsolatedPorts(targetDir, recordedPorts = {}, unchangedClause = UNCHANGED_NOTHING_AT_ALL) {
  ...
  // The generated isolated compose is JSON rendered into a `.yml` under a
  // `# GENERATED FILE — DO NOT EDIT.` header comment (cinatra#2654 D1) — YAML
  // 1.2 is a JSON superset, so it parses straight back once that header is
  // skipped. `parseIsolatedComposeDoc` is the SAME reader the in-place
  // regeneration uses; a plain `JSON.parse` of the raw text chokes on the
  // header line and reads every CLI-generated file as unparseable.
  const doc = parseIsolatedComposeDoc(raw);
...
async function reconvergeIsolated({ targetDir, opts, resolvedSha, row, log = console.log, deps = {} }) {
  ...
  // and idempotent (it does not touch ports, so it cannot perturb the gate just
  // run above), and the bring-up below regenerates the file again after the
  ...
  // A regeneration FAILURE aborts the reconcile (cinatra#2654 D1): catching it
  // here and falling back to the OLD, unrepaired compose (as this route used to)
  // is exactly the swallow that let a wayflow service with no bridge token
  // report success. `regenerateIsolatedCompose` already turns the two
  // not-re-derivable conditions into a VALIDATED skip rather than a throw (D1
  // round 3) — anything that still throws past it, including cinatra-cli#237's
  // CONCURRENT_ROW_MOVE, is a genuine failure this reconcile must not paper
  // over, so it propagates uncaught.
  ...
  let effectivePorts = regen.regenerated && regen.ports ? regen.ports : (row.ports ?? {});
  if (regen.regenerated && Number.isInteger(regen.offset) && regen.offset > 0) effectiveOffset = regen.offset;
  // cinatra-cli#231 (round-2 review): whether a regeneration ran or was skipped
  // (validated), the ports this re-converge speaks are the ports the file it is
  // about to bring up publishes — never a recorded map that lags it.
 ...
 * an existing instance — the cinatra-cli#231 "no-op when nothing is missing"
 * optimisation is gone for exactly this reason: a file with every service
 * already present is precisely the shape a stale/defective wiring survives in.
 *
 * Idempotent and safe:
 ...
 *     #231 (app-URL remap) + #2654 (env-file wiring) invariants the primary
 *     generator asserts.
 *   - On success updates the recorded `ports` (enlarged) + persists `offset`,
 *     aborting instead of overwriting a row a concurrent writer already moved
 *     (cinatra-cli#237 round-3/4 — see the persist step below).
 *
 ...
 *
 * @returns {Promise<{ regenerated: boolean, ports?: object, offset?: number, skipped?: string }>}
 */
export async function regenerateIsolatedComposeInPlace({ targetDir, row, log = console.log, wayflow = true, deps = {} }) {
  ...
  const { config: resolvedConfig, envFilesPreserved } = resolveIsolatedComposeConfig({ targetDir, deps, log });
  // cinatra#2654 D1: an unresolvable `docker compose config` THROWS rather than
  // falling back to "the recorded file already carries the a2a-peers marker, so
  // leave it be" — cinatra-cli#231's own version of that leniency is exactly the
  // kind of silent no-op that let a defective compose survive a reconcile. The
  // old file's currency is unknowable either way; starting it unchecked is the
  // failure this whole change removes, so this propagates like any other
  // regeneration failure (see the comment on the caller in `reconvergeIsolated`).
  if (!resolvedConfig) {
    throw new Error("could not resolve `docker compose config` to regenerate the isolated compose");
  }

  // cinatra#2654 D1: no "up to date, nothing to do" short-circuit here on
  // purpose. cinatra-cli#231 (round-2 review) had this stop as soon as the file
  // carried every service the checkout declares — but a FIRST install already
  // writes every profile-gated service, so a compose baking in a stale/broken
  // env wiring read as "current" forever and no documented recovery ever
  // touched it again (D1's root cause). This reconcile ALWAYS re-renders; the
  // `changed` check below (comparing the re-derived document to the previous
  // one) still reports a true no-op accurately in the log line.
  const baseBand = parseComposePublishedPorts(resolvedConfig);
  if (baseBand.length === 0) {
...
export async function runInstall(argv = [], { log = console.log, deps = {} } = {}) {
  ...
  // Injectable seams for the three children this phase spawns, so a test can
  // pin the exact command lines the install issues without running a sync, a
  // package manager or a setup child.
  ...
  if (isDevLikeMode(opts.mode)) {
    ...
    log("- Cloning declared companion extension repos (cinatra.devExtensions)…");
    const extResult = await syncDevExtensions({
      repoRoot: targetDir,
      targetRoot: targetDir,
      // `--pinned-extensions` is the ONLY thing that puts a flag here: without
      // it the sync stays tip-tracking, exactly as it has always been. The
      // pinned path is fail-closed (an entry that cannot be pinned stops the
      // install), which is the point for a caller that cannot inspect the
      // fleet afterwards.
      argv: opts.pinnedExtensions ? ["--pinned"] : [],
      env: process.env,
      log,
    ...
    if (opts.noInstall) {
      ...
      if (opts.noSetup) {
        ...
        const setupVerdict = runSetupChild({
          targetDir,
          mode: opts.mode,
          skipDevApps: opts.skipDevApps,
          pinnedExtensions: opts.pinnedExtensions,
    ...
    if (opts.noSetup) {
      ...
      // throws here (non-zero exit, named var); soft vars only warn. Runs only on
  ...
  if (opts.previewFrontDoor) {
    // cinatra-cli#197: hand the composition the instance's EFFECTIVE endpoints —
    // the half of this install's configuration `.env.example` never writes down
    // (`REDIS_URL`, the agent-registry URLs). Derived from the SAME per-service
    // published-port map the isolated path re-points against (`deriveBand` is
    // the authoritative-band seam used in step 4b), and ONLY for the plans whose
    // endpoints this checkout's own band describes — an external/co-use instance
    // dials infra this band does not model, so nothing is derived for it.
    const effectiveEndpoints = infraPlanOwnsLocalStack(infraPlan)
      ? deriveEffectiveInstanceEndpoints({ band: deriveBand(targetDir, deps) }).values
      : {};
  ...
  // 8b. cinatra-cli#198 — the guard boundary, stated at the point it becomes
  //     relevant. A completed install leaves its own byproducts behind; the next
 ...
 */
async function bootstrapPreviewFrontDoor({
  targetDir,
  opts,
  instanceSlug = null,
  effectiveEndpoints = {},
...
async function cloneOrUpdateHost({
  targetDir,
  repoUrl,
  ref,
  alreadyCheckout,
  ...
  if (alreadyCheckout) {
    ...
    // `--no-fetch` turns off only its fetch step: the checkout is already at
    // the commit the caller put there, so the move is a local resolve.
 ...
 *  Refuses an OPERATOR-dirty working tree unless `force` (stash first); refuses
 *  a divergent/non-fast-forward move unless `force` (hard-reset). Returns the
 *  resolved HEAD sha.
 *
 *  cinatra-cli#198: dirt confined to the install's OWN byproducts
 *  (`install-byproducts.mjs`) is exempt from the refusal and restored from HEAD
 *  before the move — a completed install leaves it on every run, so refusing it
 *  broke the documented `install → install` reconcile. The exemption is reported,
 *  never silent, and the refusal is unchanged for everything else.
 *
...
export function moveExistingCheckoutToRef({
  ...
  const cap = deps.capture ?? capture;
  // cinatra-cli#198: the guard now classifies the dirt instead of only counting
  // it. `workingTreeIsDirty` stays an injectable seam for callers that only want
  // to force the refusal; when it is injected and reports dirty we cannot know
  // WHOSE dirt it is, so that dirt is treated as the operator's — i.e. the
  // pre-#198 behaviour, unchanged.
  const classify =
    deps.classifyTargetDirt ??
    (deps.workingTreeIsDirty
      ? () =>
          deps.workingTreeIsDirty()
            ? { entries: [], byproducts: [], operator: [{ code: "  ", path: "(uncommitted changes)", from: null, untracked: false }], clean: false, resettable: [] }
            : { entries: [], byproducts: [], operator: [], clean: true, resettable: [] }
      : () => classifyTargetDirt(targetDir, { captureRaw: deps.captureRaw }));

  // 1. Refuse / stash a dirty tree FIRST (before any fetch or move side effect).
  //    OPERATOR dirt refuses exactly as it always has. The install's OWN
  //    byproducts (cinatra-cli#198) do not: a completed install leaves them
  //    behind on 100% of runs, so refusing them broke the documented reconcile
  //    and trained operators to reach for the stash-everything hammer.
  const dirt = classify();
  if (dirt.operator.length > 0) {
    if (!force) {
      throw new Error(
        `Refusing to move ${targetDir}: the working tree has uncommitted changes. ` +
          ...
          dirt.operator
            .slice(0, 10)
            .map((e) => `      ${e.code.trim() || "??"} ${e.path}`)
            .join("\n") +
          (dirt.operator.length > 10 ? `\n      … and ${dirt.operator.length - 10} more` : "") +
          `\n  (The install's own byproducts are NOT part of this refusal — it protects your work only:\n` +
          byproductBoundaryLines(INSTALL_BYPRODUCT_RULES).join("\n") +
          `)`,
      );
    }
  ...
  } else if (dirt.byproducts.length > 0) {
    // Restore the tracked byproducts from HEAD so the checkout/fast-forward is
    // not blocked by "local changes would be overwritten". Safe by construction:
    // the setup phase regenerates every one of them deterministically from
    // committed inputs later in this same run.
    for (const line of byproductExemptionLines(dirt.byproducts, INSTALL_BYPRODUCT_RULES)) log(line);
    if (dirt.resettable.length > 0) {
      const restore = runGit(["checkout", "HEAD", "--", ...dirt.resettable]);
      if (restore.status !== 0) {
        throw new Error(
          `Could not restore this install's own byproducts (${dirt.resettable.join(", ")}) before moving ` +
            `${targetDir}: ${(restore.stderr ?? "").trim()}. Re-run with --force to stash them instead.`,
        );
      }
    }
  }
  ...
  if (!targetCommit) {
    throw new Error(
      fetch
        ...
        : // Fail closed, naming the ref: a move asked to reach no remote can only
          // land on a commit the checkout already has, and quietly staying where
          // it is would report a commit the caller did not ask for.
          `Could not resolve ${isTag ? "tag" : "ref"} "${ref}" to a commit WITHOUT fetching. ` +
            `--no-fetch moves ${targetDir} only to a commit it already has — fetch "${ref}" into it ` +
            `first, or drop --no-fetch.`,
    );
  }
