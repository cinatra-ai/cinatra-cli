...
export const COMMAND_DESCRIPTORS = [
  ...
  // and port pool below stay the single implementation.
  {
    ...
    match: "command+mode+sub",
    summary:
      "Build + boot a LOCAL non-production image at a resolved SHA (CINATRA_RUNTIME_MODE=production, provenance local-image:<sha>); health-gates on /api/health. An image already present for that SHA is reused instead of rebuilt (--rebuild / --force-build forces a build). Before building, every container-dialed loopback endpoint (database, Redis, Nango, the agent registry) is VERIFIED to belong to this instance — a host port held by another stack is refused with the holder named, never silently composed; CINATRA_PREVIEW_ENDPOINT_OWNERSHIP=warn downgrades that refusal to a printed finding. The image build is bounded (default 90m); raise it on a slow/cold host with CINATRA_PREVIEW_BUILD_TIMEOUT_MS (ms, 1000..21600000). A build reporting 'JavaScript heap out of memory' needs the other lever: CINATRA_PREVIEW_BUILD_MEMORY_MB (MB, 256..65536; unset keeps the checkout's own) sets the build's V8 old-space limit — it does not bound Turbopack's native allocator, total container memory, or build concurrency. A NATIVE 'cannot allocate memory' is the other two: CINATRA_PREVIEW_BUILD_CPUS (1..256) sets the build's page-data worker COUNT directly — <n> IS the worker count, so 3 gives three workers, not two; only the UNSET default is derived, as os.cpus().length - 1, which a docker --cpus cap does not change. That count bounds what runs AFTER compile, so it does not fix a death DURING compile, and CINATRA_PREVIEW_BUILD_BUNDLER=turbopack|webpack picks the bundler; webpack fails on the V8 heap CINATRA_PREVIEW_BUILD_MEMORY_MB can move, the default fails on native memory it cannot. Neither removes the checkout's documented builder-memory floor. CINATRA_PREVIEW_BUILD_TYPECHECK=1 restores the in-build tsc that the forwarded CI=true skips. --bind <address> publishes the container's host port on ONE interface (--bind 127.0.0.1 for loopback-only) instead of every interface; unset is unchanged (0.0.0.0), CINATRA_PREVIEW_BIND_HOST is the env form and the flag wins, and the resolved bind is recorded on the preview's row so refresh and start --recreate re-publish on the same interface. CINATRA_PREVIEW_BUILD_CACHE=auto|off picks the builder: auto builds through docker buildx with a local layer cache when buildx is installed, so a warm rebuild reuses the unchanged layers (they report CACHED under --progress=plain), and falls back to the classic builder when it is not; off pins the classic builder. The cache lives in ~/.cinatra/preview-build-cache (CINATRA_PREVIEW_BUILD_CACHE_DIR moves it) and is pruned by deleting that directory.",
  },
  {
    id: "preview.refresh",
  ...
  },
  {
    ...
    match: "command+mode",
    summary:
      "Manage the shared WayFlow agent runtime dev container (start|stop) via the compose `wayflow` profile; start regenerates the bridge-token env first. With `--instance <name> --runtime-port <n> [--app-url <scheme://host:port>] [--image <tag>|--rebuild] [--container <name>] [--bind <address>]` it starts or stops ONE instance's own runtime container instead, on the port you name — published on this machine's loopback unless `--bind` names another interface by its IP address — and under the name you give it (else `cinatra-instance-<name>-wayflow-1` in compose project `cinatra-instance-<name>`), handed that instance's bridge token and context attest key from its own .env.local by key (a missing one is refused by name), health-gated on the runtime answering AND the container reaching that instance's app at the callback address — asked inside the container with node, or python3 when the image has no node — which is the `--app-url` you wrote, exactly as you wrote it, so it must be reachable from inside the container.",
  },
  {
    id: "dev.a2a",
    path: ["instance", "a2a"],
