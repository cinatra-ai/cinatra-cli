...
export const COMMAND_DESCRIPTORS = [
  ...
  },
  {
    ...
    match: "command+mode",
    summary:
      "Manage the shared WayFlow agent runtime dev container (start|stop) via the compose `wayflow` profile; start regenerates the bridge-token env first. With `--instance <name> --runtime-port <n> [--app-url <scheme://host:port>] [--image <tag>|--rebuild] [--container <name>]` it starts or stops ONE instance's own runtime container instead, on the port you name and under the name you give it (else `cinatra-instance-<name>-wayflow-1` in compose project `cinatra-instance-<name>`), health-gated on the runtime answering AND the container reaching that instance's app at the callback address — which is the `--app-url` you wrote, exactly as you wrote it, so it must be reachable from inside the container.",
  },
  {
    id: "dev.a2a",
    path: ["instance", "a2a"],
