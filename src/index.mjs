...
Commands:
  ...
  (alias: upgrade)  [default] — \`npm install -g @cinatra-ai/cinatra@latest\`; or
                    ...
                    then reconcile deps + dev DB. Non-interactively (piped/CI) it
                    NEVER prompts: defaults to the instance update (script
                    back-compat) unless --cli. Refuses a dirty/divergent tree
                    unless --force (which stashes / hard-resets, like install).
...
//                             (today's behavior). The git-move primitives are
//                             lazy-imported from the dependency-light install.mjs
//                             (same pattern as the install handler).
//
